...
export default function Home() {
  ...
  // Update invoice number after mount
  useEffect(() => {
    setMounted(true)
  ...
  useEffect(() => {
    ...
    if (draft) {
      ...
      if (draft.dueDate && typeof draft.dueDate === 'string') {
      ...
      if (!draft.items || !Array.isArray(draft.items) || draft.items.length === 0) {
        draft.items = [{ description: "", quantity: 0, rate: 0 }];
      }
      
      // Ensure other required fields have defaults
      draft.tax = draft.tax ?? 0;
      draft.shipping = draft.shipping ?? 0;
      draft.discount = draft.discount ?? 0;
      draft.currency = draft.currency || "EUR";
