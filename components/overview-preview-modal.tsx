...
export function OverviewPreviewModal({
  totalIncome,
  totalExpenses,
  netIncome,
  monthlyIncome,
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      printWindow.document.write(`
        ...
        <html>
          <head>
            <title>Print Financial Overview</title>
  ...
  const handleExportPDF = async () => {
    if (previewRef.current) {
      try {
        ...
        const fileName = `financial-overview-${format(new Date(), 'yyyy-MM-dd')}.pdf`;
        pdf.save(fileName);

        toast({
          title: "Export Successful",
          description: `Financial overview exported as ${fileName}`,
        });
      } catch (error) {
        ...
        <DialogContent className="max-w-4xl p-0">
          <div className="flex flex-col h-[90vh]">
            <div className="flex justify-between items-center p-4 border-b">
              <DialogTitle>Financial Overview Report Preview</DialogTitle>
            ...
            <div className="flex-1 overflow-auto">
              <div ref={previewRef} className="p-8">
                <h1 className="text-2xl font-bold mb-4">Financial Overview Report</h1>
                <p className="text-gray-600 mb-6">Generated on: {format(new Date(), 'MMM dd, yyyy HH:mm')}</p>
                
                <div className="mb-6 p-4 border rounded-md bg-gray-50">
                  <h2 className="text-lg font-semibold mb-2">Summary</h2>
                  <p className="text-sm">Net Balance: ₹{netIncome.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                  ...
                  <p className="text-sm">Monthly Net Income ({currentMonth}): ₹{monthlyNetIncome.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                  <p className="text-sm">Monthly Income ({currentMonth}): ₹{monthlyIncome.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                  <p className="text-sm">Monthly Expenses ({currentMonth}): ₹{monthlyExpenses.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                </div>

                <h2 className="text-lg font-medium text-gray-900 mb-4">Monthly Cash Flow Trend</h2>
                ...
                {recentTransactions.length > 0 ? (
                  ...
                  <p className="text-center py-8 text-gray-500">No recent transactions to display for this report.</p>
                )}
              </div>
            </div>
