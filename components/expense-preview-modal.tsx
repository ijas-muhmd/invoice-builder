...
export function ExpensePreviewModal({ expenses, expenseCategories }: ExpensePreviewModalProps) {
  ...
  const totalFilteredExpenses = expenses.reduce((sum, exp) => sum + exp.inrAmount, 0);
            ...
            <div className="flex-1 overflow-auto">
              <div ref={previewRef} className="p-8">
                ...
                <div className="mb-6 p-4 border rounded-md bg-gray-50">
                  ...
                  <p className="text-sm">Total Filtered Expenses: ₹{totalFilteredExpenses.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                  <p className="text-sm">Number of Records: {expenses.length}</p>
                </div>
                ...
                {expenses.length > 0 ? (
                    ...
                    <tbody>
                      {expenses.map(exp => (
                        <tr key={exp.id} className="border-b border-gray-200">
                          <td className="py-2 px-4">{format(new Date(exp.date), 'MMM dd, yyyy')}</td>
                          <td className="py-2 px-4">{exp.title}</td>
                          <td className="py-2 px-4">{expenseCategories.find(cat => cat.id === exp.category)?.name || 'N/A'}</td>
                          <td className="py-2 px-4">₹{exp.inrAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                  ...
                  <p className="text-center py-8 text-gray-500">No expenses to display for this report.</p>
                )}
              </div>
            </div>
